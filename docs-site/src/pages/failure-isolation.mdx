...
[deferred contribution](/server-rendering#a-deferred-contribution) loads. Unset
...
3. The client re-renders that one contribution, where the class boundary
   finally catches it and `Failed` takes over.
...
[RSC server host](/server-rendering#a-server-host-in-userland) mapping a
...
It throws outside a contribution, which is what makes it usable as the key to a
per-plugin store, logger or settings namespace.
