 ...
 * @template Props
 * @param {import('./types').SlotConfig} [config]
 * @returns {import('./types').Slot<Props>}
 */
export function createSlot(config) {
  /** @type {import('./types').SetFills | null} */
  let _setFills = null
  let nextKey = 0

  /**
   * @type {import('./types').Slot<Props>}
   */
  ...
  /**
   * @type {import('./types').Slot<Props>["Host"]}
   */
  const Host = (props) => {
    const [fills, setFills] = React.useState(
      /** @type {(import('./types').Fill | null)[]} */ [],
    )
